import { CSS2DObject, CSS3DObject, CSS3DRenderer, CSS3DSprite } from 'three/examples/jsm/Addons.js';
 ...
 * @returns
 */
export const createMarkerByDomId = (domID) => {
  // 创建div元素(作为标签)
  ...
  div.style.pointerEvents = 'none'; //避免HTML标签遮挡三维场景的鼠标事件
  // 设置HTML元素标签在three.js世界坐标中位置
  // label.position.set(label.position.x / 2, label.position.y / 2, label.position.z);
  //缩放CSS3DSprite模型对象
  // label.scale.set(0.2, 0.2, 0.2); //根据相机渲染范围控制HTML 3D标签尺寸
  // label.rotateY(Math.PI / 2); //控制HTML标签CSS3对象姿态角度
  // label.rotateX(-Math.PI/2);
