...
export function loadImage(src: string): any {
  ...
  let url = '';
  // @webglCity/assets/background/6.jpg
  const match = src.match(regex);
  // 根据项目目录查找路径
  if (match) {
    const firstPart = match[1]; // 匹配到的第一个部分
    url = `/src/project/${firstPart}/${srcArr.join('/')}`;
    return url;
  }
  if (firstStr === '@') {
    url = `/src/${srcArr.join('/')}`;
  ...
  return new Promise((resolve, resject) => {
    fetch(url).then((res) => {
      if (res.status == 200) {
        ...
        resject(res.url);
        console.error(`${src}路径的图片没有获取成功，请检查路径是否正确、文件名是否正确！`);
      }
    });
  });
}
