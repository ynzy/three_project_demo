...
export class Points {
  ...
  pointList: any[];
  points: THREE.Points<THREE.BufferGeometry<THREE.NormalBufferAttributes>, THREE.PointsMaterial> | undefined;
  setPosition: any;
  setAnimation: any;
  url: any;
  size: number;
  ...
  constructor(scene: THREE.Scene, { url, range, count, setAnimation, setPosition, size, opacity }: IPoints) {
    this.scene = scene;
    // 范围
    this.range = range;
    // 个数
